...
const PORT = process.env.PORT || 4000;
const uri = process.env.MONGODB_URI;
const CRICAPI_KEY = process.env.CRICAPI_KEY || 'c857b791-f616-45fa-8793-23efeef8b027';
...
async function loadTeamsFromDb() {
  teams = await teamsCollection
    .find({})
    .sort({ points: -1, lastScoredAt: -1, createdAt: 1 })
    .toArray();
  app.locals.teams = teams;
  return teams;
}
...
app.get('/api/match_squad', async (req, res) => {
  ...
  try {
    ...
    const squadData = Array.isArray(squadResponse.data?.data) ? squadResponse.data.data : [];
    let players = squadData[0]?.players || [];

    if (team === '2') {
      players = squadData[1]?.players || players;
    }

    players = players.map((p) => ({
      pid: p.pid || p.id,
      name: p.name || p.short_name,
      short_name: p.short_name || (p.name ? p.name.split(' ').slice(-2).join(' ') : ''),
      credit: p.credit,
      role: p.role || p.type || 'ALL',
    }));
...
app.post('/api/teams', async (req, res) => {
  const { ownerName, teamName, match, matchId, players } = req.body;
  ...
  const resolvedMatch =
    typeof match === 'object' && match !== null
      ...
      : {
          id: resolvedMatchId,
          name: typeof match === 'string' ? match : 'Practice',
        };
  ...
  try {
    ...
    const iplMatches = matches
      .filter(
        (m) =>
          m?.series?.includes('Indian Premier League') ||
          ['CSK', 'MI', 'RCB', 'KKR', 'SRH', 'DC', 'LSG', 'PBKS', 'RR', 'GT'].some(
            (team) =>
              m.t1s?.includes(team) ||
              m.t2s?.includes(team) ||
              m.t1?.includes(team) ||
      ...
      .map((m) => ({
        ...
        fantasyEnabled: true,
        matchStarted: m.ms === 'live',
        matchEnded: Boolean(m.statusSummary?.includes('Complete')),
      }))
      .sort((a, b) => new Date(b.dateTimeGMT) - new Date(a.dateTimeGMT));

    res.json(iplMatches.slice(0, 25));
  } catch (error) {
    console.error('eCricScore error:', error.message);
    res.status(503).json({ error: 'Matches unavailable' });
  }
...
async function autoScoreRecentMatches() {
  ...
  const allTeams = await loadTeamsFromDb();
  const teamsByMatch = new Map();

  for (const team of allTeams) {
    const savedMatchId = team.matchId || team.match?.id || (typeof team.match === 'string' ? team.match : null);
    if (!savedMatchId) continue;
    if (!teamsByMatch.has(savedMatchId)) teamsByMatch.set(savedMatchId, []);
    teamsByMatch.get(savedMatchId).push(team);
  }

  for (const [matchId, matchTeams] of teamsByMatch.entries()) {
    try {
      const response = await axios.get('https://api.cricapi.com/v1/match_points', {
        params: {
          apikey: CRICAPI_KEY,
          id: matchId,
          ruleset: 0,
      ...
      const matchPoints = payload.data || {};
      if (!payload.status || !matchPoints) continue;

      const completed = isCompletedMatch(matchPoints);
      const playerStatsMap = completed ? getFantasyStatsForMatch(matchPoints) : new Map();

      for (const savedTeam of matchTeams) {
        if (!completed) {
          if (savedTeam.scoreStatus !== 'pending') {
            savedTeam.scoreStatus = 'pending';
            await saveTeamToDb(savedTeam);
          }
          continue;
        }

        const playerStats = buildSelectedPlayerStats(savedTeam.players, playerStatsMap);
        const totalPoints = calculateTeamFantasyPoints(savedTeam.players, playerStats);

        if (
          totalPoints !== (savedTeam.points || 0) ||
          JSON.stringify(playerStats) !== JSON.stringify(savedTeam.playerStats || {}) ||
          savedTeam.scoreStatus !== 'completed'
        ) {
          savedTeam.points = totalPoints;
          savedTeam.playerStats = playerStats;
          savedTeam.scoreStatus = 'completed';
          savedTeam.lastScoredAt = new Date().toISOString();
          await saveTeamToDb(savedTeam);
          updatedTeams += 1;
        }
      }
    } catch (error) {
      console.error(`Fantasy points failed for match ${matchId}:`, error.message);
    }
  }
...
function isCompletedMatch(matchPoints) {
  const statusText = [
    matchPoints.status,
    matchPoints.matchStatus,
    matchPoints.statusSummary,
    matchPoints.msg,
  ]
    .filter(Boolean)
    .join(' ')
  ...
  if (statusText.includes('complete') || statusText.includes('completed') || statusText.includes('result')) {
    return true;
  }

  return extractFantasyEntries(matchPoints).length > 0;
}
...
function extractFantasyEntries(matchPoints) {
  const sources = [];

  if (Array.isArray(matchPoints?.points)) sources.push(matchPoints.points);
  if (Array.isArray(matchPoints?.data)) sources.push(matchPoints.data);
  if (Array.isArray(matchPoints?.teamInfo)) {
    matchPoints.teamInfo.forEach((team) => {
      if (Array.isArray(team?.points)) sources.push(team.points);
      if (Array.isArray(team?.players)) sources.push(team.players);
    });
  }
  if (Array.isArray(matchPoints?.teams)) {
    matchPoints.teams.forEach((team) => {
      if (Array.isArray(team?.points)) sources.push(team.points);
      if (Array.isArray(team?.players)) sources.push(team.players);
    });
  }
...
function getFantasyStatsForMatch(matchPoints) {
  const entries = extractFantasyEntries(matchPoints);
  ...
  for (const item of entries) {
    ...
    const stats = {
      points: Math.round(fantasyPoints),
      raw: item,
    };

    names.forEach((name) => {
      statsMap.set(normalizeName(name), stats);
    });
  }
...
function buildSelectedPlayerStats(selectedPlayers, statsMap) {
  const playerStats = {};

  selectedPlayers.forEach((selectedName) => {
    const normalized = normalizeName(selectedName);
    let match = statsMap.get(normalized);

    if (!match) {
      for (const [savedName, savedStats] of statsMap.entries()) {
        if (savedName.includes(normalized) || normalized.includes(savedName)) {
          match = savedStats;
          break;
        }
      }
    ...
    playerStats[selectedName] = match || { points: 0, raw: null };
  });

  return playerStats;
}

function calculateTeamFantasyPoints(players, stats) {
  return players.reduce((total, playerName) => total + (Number(stats[playerName]?.points) || 0), 0);
}
